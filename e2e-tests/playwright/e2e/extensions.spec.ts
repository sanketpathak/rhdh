...
test.describe("Admin > Extensions", () => {
  ...
  test.describe("Extensions > Installed Plugin", () => {
    ...
    test("Installed packages page", async ({ page }, testInfo) => {
      ...
      await expect(
        page.locator("div").filter({ hasText: /^1-10 of 16$/ }),
      ).toBeVisible();
      await page.getByRole("button", { name: "Next Page" }).click();
      await expect(
        page.locator("span").filter({ hasText: "-16 of 16" }),
      ).toBeVisible();
    });

    test("Analytics provider segment package sidebar for CI", async ({
      page,
    }) => {
      ...
      await page
        ...
        .click();
      await expect(
        page.getByRole("heading", { name: "Analytics Provider Segment" }),
      ).toBeVisible();
      await page
        .getByLabel(
          "You don't have permission to install plugins or view their configurations.",
        )
        .hover();
      await uiHelper.verifyTextInTooltip(
